...
let banknotes = document.querySelectorAll("[src$='rub.jpg']"); // РљРѕР»Р»РµРєС†РёСЏ (РљР°Рє Р±С‹ РјР°СЃСЃРёРІ)
let zIndex = 1;
for(let i=0; i<banknotes.length; i++){ // РџРµСЂРµР±РёСЂР°РµРј РєРѕР»Р»РµРєС†РёСЋ
  let banknote = banknotes[i]; // Р—Р°РїРёСЃС‹РІР°РµРј РѕС‡РµСЂРµРґРЅРѕР№ СЌР»РµРјРµРЅС‚ РєРѕР»Р»РµРєС†РёРё РІ РїРµСЂРµРјРµРЅРЅСѓСЋ
  banknote.onmousedown = function(e){
    ...
    this.onmouseup = function(){
      ...
      let bill_acc_top = bill_acc.getBoundingClientRect().top; // Р’РµСЂС… РєСѓРїСЋСЂРѕРїСЂРёС‘РјРЅРёРєР°
      let bill_acc_bottom = bill_acc.getBoundingClientRect().bottom - (bill_acc.getBoundingClientRect().height*2/3);
      let bill_acc_left = bill_acc.getBoundingClientRect().left;
      let bill_acc_right = bill_acc.getBoundingClientRect().right;
      let banknote_top = this.getBoundingClientRect().top; // Р’РµСЂС… РєСѓРїСЋСЂС‹
